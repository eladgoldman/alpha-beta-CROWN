...
def calzone_mnist(spec, pixels_group):
    ...
    # Load entire dataset.
    testloader = torch.utils.data.DataLoader(test_data, batch_size=1, shuffle=False, num_workers=1)
    ...
    for i in range(X.size(0)):
        data_max = data_max_res[i].view(-1)
        data_min = data_min_res[i].view(-1)
...
def mnist_loader():
    ...
    test_data = datasets.MNIST(database_path, train=False, download=True, transform=transforms.Compose([transforms.ToTensor(), normalize]))
    # Load entire dataset.
    testloader = torch.utils.data.DataLoader(test_data, batch_size=100, shuffle=False, num_workers=1)
    X, labels = next(iter(testloader))

    return X, labels, None, None, None